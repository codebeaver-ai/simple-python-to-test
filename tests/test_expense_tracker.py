...
class TestExpenseTracker(unittest.TestCase):
    ...
    def test_add_expense_with_whitespace_in_category(self):
        """
        Test that adding an expense with a valid category (including whitespace)
        is successful, and the category is stored as-is.
        """
        tracker = ExpenseTracker()

        # Add an expense with a valid category (food)
        ...
        # Verify that we can retrieve this expense using the category
        food_expenses = tracker.get_expenses_by_category("food")
        self.assertEqual(len(food_expenses), 1)
        self.assertEqual(food_expenses[0], added_expense)

        # Test that adding an expense with whitespace in the category fails
        with self.assertRaises(ValueError):
            tracker.add_expense(50.0, "  food  ", "Dinner")

    def test_add_expense_with_whitespace_in_category(self):
        """
        Test that adding an expense with a valid category is successful,
        and that adding an expense with whitespace in the category fails.
        """
        tracker = ExpenseTracker()

        # Add an expense with a valid category (without whitespace)
        self.assertTrue(tracker.add_expense(100.0, "food", "Lunch"))
