...
class TestExpenseTracker(unittest.TestCase):
    ...
    def test_add_expense_with_whitespace_in_category_raises_error(self):
        """
        Test that adding an expense with leading/trailing whitespace in the category
        raises a ValueError due to the category not being recognized.
        """
        tracker = ExpenseTracker()

        # Attempt to add an expense with whitespace in the category
        with self.assertRaises(ValueError) as context:
            tracker.add_expense(100.0, "  food  ", "Lunch")

        self.assertIn("Category must be one of:", str(context.exception))

    def test_add_expense_with_valid_category_succeeds(self):
        """
        Test that adding an expense with a valid category (without whitespace)
        is successful.
        """
        tracker = ExpenseTracker()

        # Add an expense with a valid category
