...
class TestExpenseTracker(unittest.TestCase):
    ...
    def test_add_expense_with_whitespace_in_category(self):
        """
        Test that adding an expense with a category name that contains whitespace
        is successful when the stripped category name is valid.
        ...
        self.assertTrue(tracker.add_expense(100.0, "  food  ".strip(), "Lunch"))

        # Check if the expense was added successfully
        self.assertEqual(len(tracker.expenses), 1)

        # Verify that the category was stored in lowercase without leading/trailing whitespace
        added_expense = tracker.expenses[0]
        self.assertEqual(added_expense["category"], "food")
