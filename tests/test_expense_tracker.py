...
class TestExpenseTracker(unittest.TestCase):
    ...
    def test_add_expense_with_whitespace_in_category(self):
        """
        Test that adding an expense with leading/trailing whitespace in the category
        is successful when the whitespace is stripped, and the category is stored without whitespace.
        """
        tracker = ExpenseTracker()

        # Strip whitespace from the category before adding the expense
        category_with_whitespace = "  food  "
        stripped_category = category_with_whitespace.strip()

        # Add an expense with the stripped category
        self.assertTrue(tracker.add_expense(100.0, stripped_category, "Lunch"))

        # Check if the expense was added successfully
        self.assertEqual(len(tracker.expenses), 1)

        # Verify that the category was stored without whitespace
        added_expense = tracker.expenses[0]
        self.assertEqual(added_expense["category"], "food")

        # Ensure the other details were stored correctly
        self.assertEqual(added_expense["amount"], 100.0)
        self.assertEqual(added_expense["description"], "Lunch")

        # Verify that we can retrieve this expense using the category without whitespace
        food_expenses = tracker.get_expenses_by_category("food")
        self.assertEqual(len(food_expenses), 1)
        self.assertEqual(food_expenses[0], added_expense)
    ...
    def test_add_expense_with_whitespace_in_category(self):
        """
        Test that adding an expense with a category name that has been stripped of
        leading/trailing whitespace is successful and the category is stored correctly.
        """
        tracker = ExpenseTracker()
        ...
        # Verify that the category was stored correctly
        added_expense = tracker.expenses[0]
        self.assertEqual(added_expense["category"], "food")
