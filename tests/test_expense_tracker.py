...
def test_add_expense_with_whitespace_in_category(self):
    ...
    is successful when the trimmed category matches an existing category.
    The category should be stored in lowercase without leading/trailing whitespace.
    """
    tracker = ExpenseTracker()

    # Add an expense with whitespace in the category
    self.assertTrue(tracker.add_expense(100.0, "  FOOD  ", "Lunch"))

    # Check if the expense was added successfully
    self.assertEqual(len(tracker.expenses), 1)

    # Verify that the category was stored in lowercase without whitespace
    added_expense = tracker.expenses[0]
    self.assertEqual(added_expense["category"], "food")
    ...
    # Test that adding an expense with an invalid category (even after trimming) raises an error
    with self.assertRaises(ValueError):
        tracker.add_expense(50.0, "  invalid_category  ", "Invalid expense")