...
class TestInvestmentTracker:
    ...
    def test_calculate_overall_spending_with_multiple_transactions(self):
        ...
        tracker.record_transaction(50.25, "transport", "Gas")
        tracker.record_transaction(200.75, "utilities", "Electricity bill")
        tracker.record_transaction(75.00, "entertainment", "Movie tickets")

        # Calculate the total spending
        total_spending = tracker.calculate_overall_spending()

        # Calculate the expected total
        expected_total = 100.50 + 50.25 + 200.75 + 75.00

        # Assert that the calculated total matches the expected total
        assert total_spending == pytest.approx(expected_total), f"Expected total spending to be {expected_total}, but got {total_spending}"

    def test_calculate_overall_spending_with_no_transactions(self):
        # Create an instance of InvestmentTracker
        tracker = InvestmentTracker()

        # Calculate the total spending without adding any transactions
        total_spending = tracker.calculate_overall_spending()

        # Assert that the total spending is 0 when no transactions are recorded
        assert total_spending == 0, f"Expected total spending to be 0, but got {total_spending}"
