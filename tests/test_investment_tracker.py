...
def test_record_transaction_with_string_amount(self):
    """
    Test that recording a transaction with a string amount raises a ValueError.

    This test covers:
    1. Attempting to record a transaction with a numeric amount as a string
    2. Verifying that a ValueError is raised
    3. Ensuring that no transaction is added to the expenses list
    """
    tracker = InvestmentTracker()
    ...
    # Verify that no transaction was added to the expenses list
    assert len(tracker.expenses) == 0